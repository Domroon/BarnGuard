  ...
  return (
    <>
      <ImageList rowHeight={200} cols={2}>
        {items
          ? items.map((item) => (
              <ImageListItem key={item.videoname} value={item.videoname}>
                ...
                <ImageListItemBar
                  ...
                  actionIcon={
                    <Link
                      to={{
                        pathname: "/SingleVideo/",
                        state: `${item.videoname}`,
                      }}
                    >
                      <IconButton onClick={setVideoname(value)} aria-label={`info about ${item.title}`}>
